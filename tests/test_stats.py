...
from lmo.stats import order_stats
...
def test_order_stats_sorted_concomitant_2d(x):
    x_mn = np.stack((x, x + 1))
    x_mk = order_stats(x_mn, -x, axis=-1)

    assert x_mk.shape == x_mn.shape
    assert np.all(x_mk[:, :-1] >= x_mk[:, 1:])

    x_nm = x_mn.T
    x_km = order_stats(x_mn.T, -x, axis=0)
...
def test_order_stats_fweights_const(x, f):
    ...
    x_k = order_stats(x)
    x_l = order_stats(x, fweights=fweights)
...
def test_order_stats_aweights_const(x, a):
    ...
    x_k = order_stats(x)
    x_l = order_stats(x, aweights=aweights)
